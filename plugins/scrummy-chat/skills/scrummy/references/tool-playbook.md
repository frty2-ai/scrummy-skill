...
- **Connections are consented, not configured.** A connection is made by the user signing in and ticking workspaces, so what you can reach is what they approved. A workspace missing from `list_projects` was not hidden by a filter you can lift; it was left out, and the fix is for them to reconnect and tick it.
