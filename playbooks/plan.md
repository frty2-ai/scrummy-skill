...
3. **Choose the container.** Cycles when the team runs sprints and the server exposes cycle tools (put the work in the current or next cycle; create the next one if it does not exist, on the same cadence as the last). Modules for a body of work with its own window (a campaign, a client phase, an epic). Both when the work is a module delivered across several cycles. A solo developer usually wants neither; a parent item with children is enough.
